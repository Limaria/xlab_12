document.getElementById('addProduct').addEventListener('click', async() => {
    ...
    if(nameJ){
        const res = await fetch('/app/products',{
            method: 'POST', //отвечает за создание
            headers: { //список заголовков
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({  //данные для публикации(передаем объект в формате строки)
...
function ProductsToHTML({id, name, bought}){
    ...
    productList.insertAdjacentHTML('beforeend', `<div class="form-check" id="product${id}">
